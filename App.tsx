...
const App: React.FC = () => {
  ...
  const executeSearch = async (searchTerm: string) => {
    ...
    try {
      const newHistory = [
        { query: cleanTerm, timestamp: Date.now() },
        ...history.filter(h => h.query !== cleanTerm)
      ].slice(0, 10);
      setHistory(newHistory);

      const data = await searchNcm(cleanTerm);
      setResults(data);
      if (data.length === 0) {
        setError("Não encontramos resultados para este termo. Tente descrever o produto de forma diferente.");
      }
    } catch (err: any) {
      console.error("Erro na busca:", err);
      setError("Ocorreu uma falha na conexão. Por favor, tente novamente.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="h-screen flex flex-col md:flex-row bg-slate-50 text-slate-900 overflow-hidden">
      ...
      <nav className="w-full md:w-64 bg-slate-900 text-slate-300 p-4 md:p-6 flex flex-row md:flex-col shrink-0 border-b md:border-r border-slate-800 z-20 overflow-x-auto md:overflow-x-visible no-scrollbar">
        ...
        <div className="hidden md:block mt-auto space-y-4">
          ...
          <div className="px-1">
            <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest mb-1">Aviso Legal</p>
